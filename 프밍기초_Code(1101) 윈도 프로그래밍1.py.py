...
# 4행을 실행하면 윈도창 화면에 출력 ( 베이스 윈도를 window 변수에 넣음)
...
# PhotoImage에서 절대 경로로 파일을 지정
photo = PhotoImage(file="C:/Python/CookPython/gif/dog.gif")
label1 = Label(window, image=photo)  # Label 위젯에 이미지를 추가
label1.pack()
window.mainloop()
...
for btn in btnList : # 준비한 버튼 리스트를 화면에 출력
    btn.pack( side = TOP, fil = X, padx = 10, pady = 10)
...
# SELF STUDY 
# random 모듈을 임포트하고 shuffle(리스트)함수를 사용하면 리스트를 임의로 뒤섞어 줌.
from tkinter import *
...
for i in range(0, 9):
    photoList[i] = PhotoImage(file="gif/" + fnameList[i])  # 파일 이름으로 이미지 로드
    btnList[i] = Button(window, image=photoList[i])  # 버튼에 이미지 설정
    
for i in range(0, 3):
    for k in range(0, 3):
        btnList[num].place(x=xPos, y=yPos)  # 버튼을 위치에 배치
        num += 1
        xPos += 70
    xPos = 0
    yPos += 70
...
# 프로그램 1 완성
# <이전>버튼이나 <다음>버튼을 누르면 사진들을 표시하는 사진앨범프로그램
...
fnameList = ["froyo.gif", "gingerbread.gif", "honeycomb.gif", "icecream.gif",
             "jellybean.gif", "kitkat.gif", "lollipop.gif", "marshmallow.gif", "nougat.gif"]
# 387행 : fnameList 변수에 사진 9장의 파일명을 저장
photoList = [None] * 9 # photoList에는 PhotoImage()함수로 생성할 변수 9개 준비
num = 0 # num은 현재 사진의 번호

## 함수 선언 부분 ##
def clickNext() : # <다음>버튼을 누르면 실행되는 함수
    global num # global num은 num전역변수를 함수안에서 사용 의미
    num += 1 # 사진 번호를 하나 증가
    if num > 8 : # 사진 번호가 최대 8, 8넘으면 0번사진 표시
        num = 0
    photo = PhotoImage(file = "gif/" + fnameList[num]) #변경된 사진 번호에 해당하는 이미지 파일로 pLabel 변경
...
btnNext = Button(window, text = "다음 >>", command = clickNext) # 버튼 누르면 이 함수와 연결

photo = PhotoImage(file = "gif/" + fnameList[0]) # 프로그램 실행 후 첫 번째 사진 표시
pLabel = Label(window, image = photo) # 프로그램 실행 후 첫 번째 사진 표시

btnPrev.place(x = 250, y = 10) # 버튼 및 이미지 위치 place(x=좌표, y=좌표)로 지정
btnNext.place(x = 400, y = 10) # 버튼 및 이미지 위치 place(x=좌표, y=좌표)로 지정
pLabel.place(x = 15, y = 50) # 버튼 및 이미지 위치 place(x=좌표, y=좌표)로 지정
